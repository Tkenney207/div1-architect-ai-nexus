
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Book, FileText, Code } from "lucide-react";
import Header from "@/components/Header";

const Documentation = () => {
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#D9D6D0' }}>
      <Header />

      {/* Content */}
      <section className="py-24">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <div className="rounded-2xl p-4 w-fit mx-auto mb-8" style={{ backgroundColor: '#E98B2A' }}>
              <Book className="h-12 w-12" style={{ color: '#F7F3ED' }} />
            </div>
            <h1 className="text-5xl font-bold mb-6" style={{ color: '#1A2B49' }}>Documentation</h1>
            <p className="text-xl max-w-3xl mx-auto" style={{ color: '#1A2B49' }}>
              Comprehensive guides and references for the Div1 Platform Suite
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <Card className="bg-white border hover:shadow-lg transition-all duration-300" style={{ borderColor: '#F7F3ED' }}>
              <CardHeader>
                <div className="rounded-xl p-3 w-fit mb-4" style={{ backgroundColor: '#E98B2A' }}>
                  <FileText className="h-6 w-6" style={{ color: '#F7F3ED' }} />
                </div>
                <CardTitle style={{ color: '#1A2B49' }}>Getting Started</CardTitle>
                <CardDescription style={{ color: '#1A2B49' }}>
                  Quick start guide to begin using the platform
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button className="w-full text-white hover:opacity-90" style={{ backgroundColor: '#E98B2A' }}>
                  Coming Soon
                </Button>
              </CardContent>
            </Card>

            <Card className="bg-white border hover:shadow-lg transition-all duration-300" style={{ borderColor: '#F7F3ED' }}>
              <CardHeader>
                <div className="rounded-xl p-3 w-fit mb-4" style={{ backgroundColor: '#E98B2A' }}>
                  <Code className="h-6 w-6" style={{ color: '#F7F3ED' }} />
                </div>
                <CardTitle style={{ color: '#1A2B49' }}>API Guide</CardTitle>
                <CardDescription style={{ color: '#1A2B49' }}>
                  Integration guides and best practices
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button className="w-full text-white hover:opacity-90" style={{ backgroundColor: '#E98B2A' }}>
                  Coming Soon
                </Button>
              </CardContent>
            </Card>

            <Card className="bg-white border hover:shadow-lg transition-all duration-300" style={{ borderColor: '#F7F3ED' }}>
              <CardHeader>
                <div className="rounded-xl p-3 w-fit mb-4" style={{ backgroundColor: '#E98B2A' }}>
                  <Book className="h-6 w-6" style={{ color: '#F7F3ED' }} />
                </div>
                <CardTitle style={{ color: '#1A2B49' }}>Tutorials</CardTitle>
                <CardDescription style={{ color: '#1A2B49' }}>
                  Step-by-step tutorials for common workflows
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button className="w-full text-white hover:opacity-90" style={{ backgroundColor: '#E98B2A' }}>
                  Coming Soon
                </Button>
              </CardContent>
            </Card>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Documentation;
